...
class FunASRVoiceRecognizer:
    ...
    def __init__(self,
        ...
        # 优先使用传入的model_path，否则从配置文件获取
        if model_path:
            self.model_path = model_path
        else:
            try:
                from utils.config_loader import config
                self.model_path = config.get_funasr_model_path()
            except ImportError:
                self.model_path = "./model/fun"
    ...
    def _load_model(self) -> bool:
        ...
        try:
            ...
            if self._model is None:
                logger.error("❌ AutoModel返回None，模型加载失败")
                return False
