...
如果您的开发环境已经安装了所有依赖（如torch、funasr等），可以使用轻量级构建脚本：
...
要求：
- Python 3.8+
- 所有依赖已通过 `requirements-nuitka.txt` 安装
- 或者在现有项目中已经使用了完整环境
...
#### 轻量级安装 (已有完整环境)
```bash
pip install -r requirements-nuitka-light.txt
```

仅包含：
- `nuitka==1.9.2` - 编译器
- `ordered-set==4.1.0` - 必需插件
- `zstandard==0.21.0` - 必需插件
...
# 轻量级构建 (推荐已有环境)
build_nuitka_light.bat        # Windows
./build_nuitka_light.sh       # Linux/macOS
```
