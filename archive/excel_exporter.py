...
class ExcelExporter:
    def __init__(self, filename: Optional[str] = None):
        ...
        # 📍 修改：不设置列名，使用模板的列名
        # 模板的列名由create_from_template方法读取
        self.columns = []  # 列名将在模板复制时读取
        self.use_template_columns = True  # 标记使用模板列名
    ...
    # 📍 新增方法：读取模板列名
    def _read_template_columns(self) -> None:
        """
        读取模板文件的列名（从第3行读取）
        """
        try:
            ...
            if columns:
                ...
                logger.info(f"读取模板列名: {self.columns}")
            else:
                # 如果模板为空，使用默认列名
                logger.warning("模板文件为空，使用默认列名")
                self.columns = ["标准序号", "Excel编号", "测量值", "时间戳", "处理文本", "语音录入编号"]

        except Exception as e:
            logger.error(f"读取模板列名失败: {e}")
            # 降级到默认列名
            self.columns = ["标准序号", "Excel编号", "测量值", "时间戳", "处理文本", "语音录入编号"]
    ...
    def _write_to_specific_row(self, row: int, voice_id: int, value: Any, **kwargs) -> None:
        ...
        try:
            ...
            # A列: 标准序号 (居中)
            cell_a = worksheet.cell(row=row, column=1, value=self.current_standard_id)
            cell_a.alignment = Alignment(horizontal='center', vertical='center')

            # F列: 时间戳 (居中)
            cell_f = worksheet.cell(row=row, column=6, value=pd.Timestamp.now().strftime("%Y-%m-%d %H:%M:%S"))
            cell_f.alignment = Alignment(horizontal='center', vertical='center')

            # G列: 语音录入ID (居中)
            cell_g = worksheet.cell(row=row, column=7, value=voice_id)
            cell_g.alignment = Alignment(horizontal='center', vertical='center')

            # 根据数据类型写入测量值 (居中)
            if isinstance(value, str):
                cell_d = worksheet.cell(row=row, column=4, value=value)  # D列: 测量值
                cell_d.alignment = Alignment(horizontal='center', vertical='center')
            else:
                cell_d = worksheet.cell(row=row, column=4, value=float(value))  # D列: 测量值
                cell_d.alignment = Alignment(horizontal='center', vertical='center')
